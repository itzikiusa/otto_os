...
//!
//! The daemon's `otto_usage::UsageEngine` records *real* token usage by tailing
//! the transcript files the agent CLIs write to disk. This module holds the
...
//!
//! Extracted from `otto-usage` (which re-exports every item under its old
//! paths) so the conversation view and the usage store read one parser. The
//! serialized shapes of `<data_dir>/usage_tailer.json` ([`CursorStore`]),
//! `usage_tailer_seen.json` ([`SeenKeys`]) and `usage_tailer_codex_totals.json`
//! ([`CodexCounterStore`]) are FROZEN — `tests/usage_formats.rs` loads a
//! checked-in copy of each, so the daemon neither double-counts nor rebuilds
//! after this move.
//!
...
/// providers and ready to be turned into an `otto_usage::UsageEvent`.
