...
fn tool_calls(f: &Folded) -> Vec<(&str, ToolKind, Option<&ToolResult>)> {
    blocks(f)
        ...
        .filter_map(|b| match b {
            Block::ToolCall {
                name, tool, result, ..
            } => Some((name.as_str(), *tool, result.as_ref())),
            _ => None,
        })
        .collect()
}
...
fn every_fixture_folds_cleanly() {
    ...
    for (sub, provider) in [
        ...
        for p in fixtures(sub) {
            ...
            assert_eq!(
                earlier.turns.len() + page.turns.len(),
                f.turns.len().min(earlier.turns.len() + page.turns.len())
            );
            n += 1;
        }
    }
    ...
    let sub_dir =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("fixtures/claude/03-agent-subagents/subagents");
...
fn claude_fixtures_pin_their_features() {
    ...
    assert!(calls
        .iter()
        .any(|(n, k, r)| *n == "Edit" && *k == ToolKind::Edit && r.is_some()));
    ...
    assert!(blocks(&f)
        .iter()
        .any(|b| matches!(b, Block::Tasks { tasks } if !tasks.is_empty())));
    ...
    assert!(f
        .artifacts
        .iter()
        .any(|a| a.kind == ArtifactKind::Pr && a.url.is_some()));
    assert!(blocks(&f)
        .iter()
    ...
    assert!(calls
        .iter()
        .any(|(n, k, r)| *n == "ToolSearch" && *k == ToolKind::Search && r.is_some()));

    let f = fold(Provider::Claude, "claude", "09-bare-string-result");
    assert!(tool_calls(&f)
        .iter()
        .any(|(_, _, r)| r.is_some_and(|r| !r.ok)));

    let f = fold(Provider::Claude, "claude", "10-write-structured-patch");
    assert!(tool_calls(&f)
        .iter()
        .any(|(_, _, r)| r.is_some_and(|r| r.patch.is_some() && r.file_path.is_some())));
    ...
    let f = fold(
        Provider::Claude,
        "claude",
        "13-system-reminder-in-tool-result",
    );
    assert!(tool_calls(&f).iter().any(|(_, _, r)| r.is_some_and(|r| r
        .text
        .as_deref()
    ...
    assert!(f
        .turns
        .iter()
        .any(|t| t.turn.system.iter().any(|n| n.kind == SystemNoteKind::Hook)));
    ...
    assert_eq!(
        ...
        Some(0),
        "1e300 is rejected, not saturated to 2^64"
    );
}
...
fn claude_subagent_tree_attaches_to_agent_calls() {
    let main =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("fixtures/claude/03-agent-subagents.jsonl");
    ...
    let f = fold_file(
        ...
        &main,
        FoldOpts {
            subagents: tree.clone(),
            ..Default::default()
        },
    )
    .unwrap();
    let subs: Vec<&Block> = blocks(&f)
        .into_iter()
...
fn codex_new_era_fixtures_pin_their_features() {
    ...
    assert!(tool_calls(&f)
        .iter()
        .any(|(_, k, r)| *k == ToolKind::Shell && r.is_some()));
    assert!(blocks(&f).iter().any(|b| matches!(b, Block::Text { .. })));
    assert!(
        f.turns
            .iter()
            .all(|t| t.turn.id.ends_with(":u") || t.turn.id.ends_with(":a")),
        "new-era ids"
    );
    ...
    assert!(tool_calls(&f)
        .iter()
        .any(|(_, k, r)| matches!(k, ToolKind::Edit | ToolKind::Write)
            && r.is_some_and(|r| r.patch.is_some())));
    assert!(f.stats.input_tokens.is_some());
    ...
    assert!(tool_calls(&f)
        .iter()
        .any(|(_, k, _)| matches!(k, ToolKind::Web | ToolKind::Read | ToolKind::Other)));
}
...
fn codex_old_era_fixtures_pin_their_features() {
    ...
    assert!(
        f.turns.iter().all(|t| t.turn.id.starts_with('r')),
        "old-era ids are r<index>"
    );
    assert!(tool_calls(&f)
        .iter()
        .any(|(n, k, r)| *n == "exec_command" && *k == ToolKind::Shell && r.is_some()));
    assert!(f.turns.iter().any(|t| t.turn.role == Role::User));
    ...
    assert!(tool_calls(&f)
        .iter()
        .any(|(n, _, _)| *n == "exec_command" || *n == "exec"));
