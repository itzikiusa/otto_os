...
async fn governed_discovery_redacts_credentials_and_does_not_grant_configuration() {
    ...
    let policy = AccessPolicy {
        ...
        rules: vec![AccessRule {
            id: "discover-only".into(),
            subject_kind: SubjectKind::User,
            subject_id: viewer.id.clone(),
    ...
    let visible = ctx
        .connections()
        .visible_connection(conn.clone(), &viewer.id)
        .await
        .unwrap();
    ...
    assert!(ctx
        .connections()
        .authorize(&conn.id, &viewer.id, "configure")
        .await
        .is_err());
    assert!(matches!(
        ctx.connections()
            .visible_connection(conn, &stranger.id)
            .await
            .unwrap_err(),
        Error::NotFound(_)
    ));
...
async fn delegated_configure_cannot_rebind_native_identity_or_disable_protection() {
    use otto_core::access::*;
    let pool = mem_pool().await;
    let root = seed_user(&pool, "owner", true, "admin").await;
    let member = seed_user(&pool, "delegated", false, "view").await;
    let ctx = TestCtx::new(pool.clone());
    let request = UpsertConnectionReq {
        name: "Restricted DB".into(),
        kind: ConnectionKind::Mysql,
        params: serde_json::json!({"host":"example.invalid","port":3306,"user":"limited"}),
        secret: None,
        first_command: None,
        section_id: None,
        environment: Some(otto_core::domain::Environment::Prod),
        read_only: Some(true),
    };
    let conn = ctx
        .svc
        .create(None, &root.id, request.clone())
        .await
        .unwrap();
    ...
    .await
    .unwrap();
    let mut cosmetic = request.clone();
    cosmetic.name = "Renamed".into();
    assert_eq!(
        ctx.svc
            .update(&conn.id, &member.id, cosmetic)
            .await
            .unwrap()
            .name,
        "Renamed"
    ...
    for field in [
        ...
        match field {
            "params" => mutation.params["user"] = serde_json::json!("hidden_admin"),
            "secret" => mutation.secret = Some("replacement".into()),
            "environment" => mutation.environment = Some(otto_core::domain::Environment::Dev),
            "read_only" => mutation.read_only = Some(false),
            _ => mutation.first_command = Some("unrestricted command".into()),
        };
        assert!(
            matches!(
                ctx.svc.update(&conn.id, &member.id, mutation).await,
                Err(Error::Forbidden(_))
            ),
            "sensitive mutation {field}"
        );
    }
    let stored = ConnectionsRepo::new(pool).get(&conn.id).await.unwrap();
    assert_eq!(stored.params, request.params);
    assert!(stored.read_only);
    assert_eq!(stored.environment, otto_core::domain::Environment::Prod);
    assert!(
        matches!(
            ctx.svc.create(None, &member.id, request).await,
            Err(Error::Forbidden(_))
        ),
        "new aliases must be provisioned by root"
    );
}
...
async fn nonroot_admin_cannot_scan_or_import_ambient_native_identities() {
    let pool = mem_pool().await;
    let admin = seed_user(&pool, "setup-admin", false, "admin").await;
    let ctx = TestCtx::new(pool);
    for (method, path, body) in [
        (
            "GET",
            "/workspaces/test/connections/import/sources",
            serde_json::json!({}),
        ),
        (
            "POST",
            "/workspaces/test/connections/import/scan",
            serde_json::json!({"source":"mysql_workbench"}),
        ),
        (
            "POST",
            "/workspaces/test/connections/import/create",
            serde_json::json!({"connections":[],"section_id":null}),
        ),
    ] {
        assert_eq!(
            call(&ctx, &admin, method, path, body).await,
            StatusCode::FORBIDDEN,
            "ambient setup endpoint {path}"
        );
    }
}
