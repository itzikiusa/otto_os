...
/// Search mode. Vectors were removed with Vault v3 — `Hybrid` and `Semantic`
/// are kept as accepted aliases (wire + Rust compat) and execute the keyword
/// path.
...
/// `kind` is one of: `keyword` (FTS/term match) or `scope` (same story).
