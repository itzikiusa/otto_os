...
use otto_core::domain::{
    AgentTask, Session, SessionKind, SessionStatus, TrailKind, TrailLevel, TrailSource,
    WorkspaceRole,
};
...
pub(crate) fn transcript_roots(data_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    (
        ...
        otto_sessions::codex_sessions_root(),
    )
}
...
pub(crate) async fn resolve_transcript(
    ctx: &ServerCtx,
    session: &Session,
...
async fn history_path_gate(
    ...
    let own = ctx
        .manager
        .list_by_workspace_for_user(wid, &user.id)
        .await
        .map_err(ApiError)?;
...
pub async fn get_transcript(
    ...
    let resolved = match resolve_transcript(&ctx, &session).await {
        ...
        Err(why) => {
            ...
            return Ok(Json(Transcript::unavailable(
                provider,
...
pub async fn touch_workspace_transcripts(
    ...
    for s in sessions {
        ...
        if require_session_owner_or_admin(&ctx, &user, &s)
            .await
            .is_err()
        {
            continue;
...
pub(crate) async fn register_work_artifact(ctx: &ServerCtx, session: &Session, a: &Artifact) {
    ...
    if let Ok(existing) = ctx.workgraph.repo().artifacts_for(&item.id).await {
        if existing
            .iter()
            .any(|e| e.reference == reference && e.reference.is_some())
        {
            return;
        }
    }
...
pub async fn get_artifact(
    ...
    let Some(path) = art.path.as_deref() else {
        return Err(ApiError(Error::Invalid(
            "artifact has no local file (a link)".into(),
        )));
    };
    ...
    if !artifact_path_allowed(&canonical, &session.cwd, &ctx.data_dir) {
        return Err(ApiError(Error::Forbidden(
            "artifact path is not permitted".into(),
        )));
    }
...
pub async fn create_task(
    ...
    if ctx.manager.is_live(&session.id)
        && !crate::agent_tasks_nudge::agent_running(&ctx, &session).await
    {
        return Err(ApiError(Error::Conflict(
            "agent not running in this terminal".into(),
        )));
    }
    ...
    if title.chars().count() > 500 {
        return Err(ApiError(Error::Invalid(
            "title is too long (500 chars max)".into(),
        )));
    }
    let description = req
        .description
    ...
    let _ = activity
        .append_trail(NewTrail {
            ...
            summary: format!(
                "Board task added: {}",
                otto_transcript::util::clip(title, 120)
            ),
            detail: None,
        })
        .await;
...
pub async fn inbox_upload(
    ...
    if !mime.starts_with("image/") || mime.matches('/').count() != 1 || mime.contains(';') {
        return Err(ApiError(Error::UnsupportedMedia(
...
/// accepted.
pub(crate) fn confine_history_path(
    ctx: &ServerCtx,
    raw: &str,
) -> Result<(PathBuf, Provider), ApiError> {
    ...
    if e2e_enabled() && raw.contains("/crates/otto-transcript/fixtures/") {
        if let (Ok(canon), Some(provider)) = (
            candidate.canonicalize(),
            otto_transcript::provider_for_path(candidate),
        ) {
            if canon.is_file() {
    ...
    for (root, provider) in [
        (claude_root, Provider::Claude),
        (codex_root, Provider::Codex),
    ] {
...
pub async fn history(
    ...
    let persisted: HashMap<Id, Option<String>> = repo
        ...
        .into_iter()
        .collect();
    let claimed_paths: HashSet<String> = repo
        .transcript_paths()
        .await
        .map_err(ApiError)?
        .into_iter()
        .collect();
    let claimed_psids: HashSet<String> = repo
    ...
    let session_paths: Vec<String> = resolved
        .iter()
        .map(|(_, r)| r.path.to_string_lossy().into_owned())
        .collect();
    ...
    let by_path: HashMap<&str, &otto_state::TranscriptIndexRow> = rows_for_sessions
        .iter()
    ...
    for r in &rows {
        if claimed_paths.contains(&r.path)
            || r.provider_session_id
                .as_deref()
                .is_some_and(|p| claimed_psids.contains(p))
        {
            continue;
        }
        ...
        out.push(HistoryEntry {
            ...
            started_at: r
                .started_at
                .clone()
                .unwrap_or_else(|| r.last_active_at.clone().unwrap_or_default()),
            last_active_at: r
                .last_active_at
                .clone()
                .unwrap_or_else(|| r.started_at.clone().unwrap_or_default()),
    ...
    out.retain(|e| {
        provider_f.is_none_or(|p| e.provider == p)
            && cwd_f.is_none_or(|c| {
                e.cwd == c || e.cwd.starts_with(&format!("{}/", c.trim_end_matches('/')))
            })
...
pub async fn history_import(
    ...
    if Provider::parse(&req.provider).is_some_and(|p| p != provider) {
        return Err(ApiError(Error::Invalid(
            "provider does not match the transcript location".into(),
        )));
    }
    let p2 = path.clone();
        ...
        .map_err(ApiError)?
    {
        ...
        let _ = repo
            .set_transcript_path(&existing.id, &path.to_string_lossy())
            .await;
    ...
    let title = peek
        ...
        .or_else(|| {
            peek.first_prompt
                .as_deref()
                .map(|p| otto_transcript::util::clip(p, 80))
        })
    ...
    repo.update_status(&session.id, SessionStatus::Reconnectable)
        ...
        .map_err(ApiError)?;
    let _ = repo
        .set_transcript_path(&session.id, &path.to_string_lossy())
        .await;
