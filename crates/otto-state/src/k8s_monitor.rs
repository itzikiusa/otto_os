...
pub struct K8sMonitorConfigRow {
    ...
    /// Keep per-request `path`/`method` labels on request + latency counters.
...
fn row_to_config(r: &sqlx::sqlite::SqliteRow) -> K8sMonitorConfigRow {
    K8sMonitorConfigRow {
        ...
        request_labels: r.try_get::<i64, _>("request_labels").unwrap_or(0) != 0,
    ...
    pub async fn upsert_config(&self, row: &K8sMonitorConfigRow) -> Result<K8sMonitorConfigRow> {
        ...
        sqlx::query(
            "INSERT INTO k8s_monitor_configs (cluster_id, enabled, interval_secs, namespaces_json,
                                              ...
                                              retention_days, series_cap, metrics_server, request_labels, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
             ON CONFLICT(cluster_id) DO UPDATE SET
                ...
                request_labels = excluded.request_labels, updated_at = excluded.updated_at",
        )
        .bind(&row.cluster_id)
        ...
        .bind(row.request_labels as i64)
        .bind(&now)
        .execute(&self.pool)
        .await
