<script lang="ts">
  ...
  async function load(id: number, path: string) {
    ...
    } catch (e) { if (request === seq) error = loadErrorText(e); }
  ...
  async function selectRevision(entry: VaultRevision) {
    ...
    } catch (e) { if (request === detailSeq) error = loadErrorText(e); }
  }
  ...
  async function restoreRevision(version: 'before' | 'after') {
    ...
    try {
      ...
      toasts.success(`Restored the ${version} version`, entry.path);
      if (request !== seq || vault.current?.id !== id || vault.wsId !== workspace) return;
      await load(id, vault.historyPath);
      await vault.refreshTree();
...
<section class="recovery" aria-label={mode === 'trash' ? 'Vault trash' : 'Vault edit history'}>
  <header><h2>{mode === 'trash' ? 'Trash' : 'Edit history'}</h2><button class="btn small" disabled={loading} onclick={() => vault.current && void load(vault.current.id, vault.historyPath)}><Icon name="refresh" size={12} /> Refresh</button></header>
  {#if error}
    <div role="alert" class="error"><span>Couldn’t load {mode === 'trash' ? 'the trash' : 'the edit history'}: {error}</span>
      <button class="btn small" disabled={loading} onclick={() => vault.current && void load(vault.current.id, vault.historyPath)}>Retry</button></div>
  {/if}
  {#if mode === 'trash'}
  ...
  {:else}
    ...
    <div class="history-layout">
      <nav aria-label="Saved revisions">
        ...
        {/each}
        {#if more}<button class="btn small" disabled={loading} onclick={() => void loadOlder()}>Load older edits</button>{/if}
      </nav>
      <div class="revision-detail">
        {#if selected}
          <h3>{selected.path}</h3>
          <div class="actions"><button class="btn small" disabled={busy || selected.before === null || currentHash === null} onclick={() => void restoreRevision('before')}>Restore before version</button><button class="btn small" disabled={busy || currentHash === null} onclick={() => void restoreRevision('after')}>Restore after version</button></div>
