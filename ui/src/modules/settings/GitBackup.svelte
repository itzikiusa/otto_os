...
<section class="git-backup" aria-label="Git backup sync">
  ...
  {#if status}
    <p class="dim">{status.branch ?? 'Detached HEAD'} · tracking {status.upstream ?? 'none'} · {status.ahead} ahead · {status.behind} behind · {status.dirty ? 'Local changes present' : 'Working tree clean'}</p>
    <button class="btn" disabled={busy} onclick={previewExport}>Preview snapshot</button>
    {#if preview}
      <div class="snapshot-preview" aria-label="Git snapshot preview">
        <strong>{changes.length} changed files</strong>
        ...
        <div class="scroll"><ul>{#each changes as change}<li><span>{change.action}</span> <code>{change.path}</code></li>{:else}<li>The snapshot matches this repository.</li>{/each}</ul></div>
        ...
        <button class="btn primary" disabled={busy || exported} onclick={writeSnapshot}>{exported ? 'Snapshot written' : 'Write snapshot'}</button>
        {#if exported}
    ...
    <h4>Import from this snapshot</h4>
...
<style>
  .git-backup { border: 1px solid var(--border); border-radius: var(--radius-m); padding: 18px; margin: 18px 0; }
  h3 { margin: 0 0 8px; font-size: 16px; }
  h4 { margin: 20px 0 8px; }
  p { font-size: 13px; line-height: 1.5; }
  label, details { font-size: 12px; }
  ...
  .snapshot-preview { margin: 14px 0; padding: 12px; background: var(--surface-2); border-radius: var(--radius-s); font-size: 12px; }
