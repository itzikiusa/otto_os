<script lang="ts">
  ...
  let { preview, busy, reviewed = $bindable(false), onrestore }: {
    ...
    <div class="preview" aria-label="Restore preview">
      ...
      <details open><summary>Included records</summary><div class="scroll"><table><thead><tr><th>Area</th><th>Records</th></tr></thead><tbody>{#each Object.entries(preview.table_counts).sort(([a], [b]) => a.localeCompare(b)) as [table, count]}<tr><td>{table.replaceAll('_', ' ')}</td><td>{count}</td></tr>{/each}</tbody></table></div></details>
...
<style>
  .preview { border-top: 1px solid var(--border); padding-top: 14px; margin-top: 14px; font-size: 12px; }
  details { margin: 10px 0; font-size: 12px; }
  summary { cursor: pointer; }
  ul, .scroll { max-height: 240px; overflow: auto; overflow-wrap: anywhere; }
  li { margin: 5px 0; }
  table { width: 100%; border-collapse: collapse; }
  th, td { padding: 5px 8px; border-bottom: 1px solid var(--border); text-align: start; }
  .reviewed { display: flex; align-items: start; gap: 7px; margin: 14px 0; }
