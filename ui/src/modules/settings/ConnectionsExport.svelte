...
<section class="connections-export" aria-label="Connection export">
  ...
  {#if loading}<p role="status">Loading export formats…</p>{:else if formats.length}
    <div class="controls">
      <label>Export format<select class="input" aria-label="Export format" bind:value={format} disabled={busy} onchange={clearResult}>{#each formats as item}<option value={item.id}>{item.label}</option>{/each}</select></label>
      <label>Connections to export<select class="input" bind:value={scope} disabled={busy} onchange={clearResult}><option value="all">All workspaces</option><option value="workspaces">Selected workspaces</option></select></label>
    </div>
    {#if scope === 'workspaces'}
    ...
    <button class="btn primary" disabled={busy || !format || (scope === 'workspaces' && selectedIds.length === 0)} onclick={prepare}>{busy ? 'Preparing export…' : includePasswords ? 'Prepare export with passwords' : 'Prepare export'}</button>
  {/if}
  {#if result}
    <div class="result" aria-label="Prepared connection export">
      ...
      <button class="btn" onclick={() => result = null}>Clear prepared export</button>
    </div>
  {/if}
  {#if error}<p class="error" role="alert">{error}</p>{#if !formats.length}<button class="btn" onclick={load}>Retry formats</button>{/if}{/if}
</section>

<style>
  .connections-export { border: 1px solid var(--border); border-radius: var(--radius-m); padding: 18px; margin: 18px 0; }
  h3 { margin: 0 0 8px; font-size: 16px; }
  p { font-size: 13px; line-height: 1.5; }
  .controls { display: flex; flex-wrap: wrap; gap: 10px; margin: 12px 0; }
  .controls label { display: flex; flex-direction: column; gap: 5px; font-size: 12px; max-width: 100%; }
  select { max-width: 100%; }
  .dim { color: var(--text-dim); }
  .password-option, .workspaces label { display: flex; align-items: start; gap: 7px; font-size: 12px; }
  .password-note { color: var(--text); }
  .workspaces { display: grid; gap: 7px; max-height: 220px; overflow: auto; margin: 12px 0; }
  .result { border-top: 1px solid var(--border); margin-top: 16px; padding-top: 8px; }
