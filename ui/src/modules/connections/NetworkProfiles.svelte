<script lang="ts">
  ...
  let endpointNames = $state<HTMLInputElement[]>([]);
  async function addEndpoint() {
    if (busy || endpoints.length >= 8) return;
    endpoints = [...endpoints, emptyEndpoint()];
    await tick();
    endpointNames[endpoints.length - 1]?.focus();
  }
  async function removeEndpoint(index: number) {
    if (busy || endpoints.length === 1) return;
    endpoints = endpoints.filter((_, i) => i !== index);
    await tick();
    endpointNames[Math.min(index, endpoints.length - 1)]?.focus();
  }
  let alive = true;
  $effect(() => () => { alive = false; });
...
<div class="manager">
  ...
  {#each endpoints as endpoint, i (i)}
    <fieldset disabled={busy}><legend>Endpoint {i + 1}</legend>
      ...
      <button class="btn small remove-endpoint" type="button" disabled={endpoints.length === 1} title={endpoints.length === 1 ? 'A profile needs at least one endpoint' : undefined} onclick={() => void removeEndpoint(i)}>Remove endpoint {i + 1}</button>
    </fieldset>
  {/each}
  <button class="btn small" type="button" disabled={busy || endpoints.length >= 8} title={endpoints.length >= 8 ? 'Up to 8 endpoints per profile' : undefined} onclick={() => void addEndpoint()}>Add endpoint</button>
...
<style>
  /* The containing sheet/panel owns scrolling: nesting a 60vh viewport here
     hid the save action and trapped touch scrolling inside the form. */
  .manager { display: grid; gap: 12px; min-width: 0; padding: 12px; border: 1px solid var(--border); border-radius: var(--radius-m); background: var(--surface); }
  label { display: grid; gap: 4px; font-size: var(--fs-s); min-width: 0; }
  fieldset { display: grid; gap: 8px; min-width: 0; margin: 0; padding: 12px; border: 1px solid var(--border); border-radius: var(--radius-s); }
  legend { padding-inline: 4px; font-size: var(--fs-s); font-weight: 600; }
  .endpoint-fields { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(160px, 100%), 1fr)); gap: 8px 12px; min-width: 0; }
  input, select { min-width: 0; width: 100%; box-sizing: border-box; padding: 6px 8px; color: var(--text); background: var(--surface-2); border: 1px solid var(--border); border-radius: var(--radius-s); font-size: var(--fs-s); }
  .profiles { display: flex; flex-wrap: wrap; gap: 8px; }
  .profiles button { max-width: 100%; white-space: normal; overflow-wrap: anywhere; }
  .remove-endpoint { justify-self: start; }
  .archive { display: flex; align-items: center; }
  .archive input { width: auto; }
  p { font-size: var(--fs-s); color: var(--text-dim); margin: 0; overflow-wrap: anywhere; }
