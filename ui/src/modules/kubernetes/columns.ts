...
const NAME: Column = { key: 'name', label: 'Name', width: 'minmax(180px, 2fr)', value: (r) => r.name };
const NS: Column = { key: 'namespace', label: 'Namespace', width: 'minmax(110px, 1fr)', value: (r) => r.namespace };
const STATUS: Column = { key: 'status', label: 'Status', width: 'minmax(110px, 1fr)', status: true, value: (r) => r.status };
const READY: Column = { key: 'ready', label: 'Ready', width: '70px', num: true, mono: true, value: (r) => r.ready ?? '' };
const RESTARTS: Column = { key: 'restarts', label: 'Restarts', width: '76px', num: true, mono: true, value: (r) => (r.restarts == null ? '' : String(r.restarts)) };
const CPU: Column = { key: 'cpu', label: 'CPU', width: '76px', num: true, mono: true, value: (r) => (r.cpu == null ? '' : formatMillicores(r.cpu)) };
const MEM: Column = { key: 'mem', label: 'MEM', width: '80px', num: true, mono: true, value: (r) => (r.mem == null ? '' : formatBytes(r.mem)) };
const NODE: Column = { key: 'node', label: 'Node', width: 'minmax(120px, 1fr)', value: (r) => r.node ?? '' };
const IP: Column = { key: 'ip', label: 'IP', width: '120px', mono: true, value: (r) => r.ip ?? '' };
const AGE: Column = { key: 'age', label: 'Age', width: '64px', num: true, mono: true, value: (r) => formatAge(r.age_seconds) };
...
const KIND_EXTRA: Partial<Record<K8sResourceKind, Column[]>> = {
  ...
  jobs: [ex('completions', 'Completions', '100px', { mono: true }), ex('duration', 'Duration', '80px', { mono: true })],
  ...
  ingresses: [ex('class', 'Class', '90px'), ex('hosts', 'Hosts', 'minmax(160px, 2fr)', { mono: true }), ex('address', 'Address', 'minmax(120px, 1fr)', { mono: true }), ex('ports', 'Ports', '80px', { mono: true })],
  configmaps: [ex('data', 'Data', '60px', { num: true, mono: true })],
  secrets: [ex('type', 'Type', 'minmax(160px, 1.4fr)', { mono: true }), ex('keys', 'Keys', 'minmax(120px, 1fr)', { mono: true })],
  ...
  events: [ex('type', 'Type', '80px'), ex('reason', 'Reason', '130px'), ex('object', 'Object', 'minmax(160px, 1.4fr)', { mono: true }), ex('message', 'Message', 'minmax(240px, 3fr)'), ex('count', 'Count', '60px', { num: true, mono: true })],
...
export function columnsFor(
  ...
  if (kind === 'pods') {
    ...
    // Events have no meaningful status; their `status` carries "last seen".
    cols.push({ ...STATUS, status: false, label: STATUS_LABEL.events ?? 'Status', width: '100px' });
