...
| [Browser](./browser.md) | A workspace-scoped web browser: reader tabs (fetch → markdown, JS-capable via a Lightpanda sidecar with plain-fetch fallback), DOM annotations you can send into a live session or save to the Vault, a CSS-selector query, page summarize, and 4 `browser_*` agent MCP tools. Every fetch is netguard-checked. | Paste a URL in the Browser tab; no setup needed. |
