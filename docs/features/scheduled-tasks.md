...
- **Workflow handoff is single-launch** — a `workflow` task launches one workflow
  run per fire and reports its node outcome. To turn a single-step task into a full
  multi-step workflow, **`POST /scheduled-tasks/{id}/convert-to-workflow`** (§10, #144)
  materializes it as a workflow + schedule trigger.
