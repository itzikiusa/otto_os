...
| GET /repos/{id}/pull-mode | ws viewer | — | `{mode: "merge"\|"rebase"\|"ff_only"}` — the mode a pull with no explicit `mode` would use, read from the repo's own git config (`pull.rebase` `true\|merges\|interactive` → rebase, else `pull.ff=only` → ff_only, else merge). Not cached: the config can change under the app. |
