...
The first GitHub Actions run subsequently exposed a Git 2.55 compatibility issue: its persisted bisect completion marker quotes `bad`, unlike earlier Git versions. The reader now accepts both exact formats. A regression reproduced the failure before the correction; the corrected Git crate suite passes **207 tests**, including completed-session reloads and inconclusive skipped-candidate coverage. These two added tests are separate from the earlier workspace total above.

The next CI run passed those 207 Git tests and the server tests, then exposed a pre-existing intermittent deadlock in `input_records_capture_probe`: the test held a DashMap read guard across another input's write lookup. Its inspection guard now ends before further input, with a nonblocking assertion that reproduced the failure deterministically before the fix. This correction changes test code only; the session crate suite passes 146 tests (3 ignored), and strict targeted Clippy passes. CodeQL's 14 annotations were individually traced and dismissed with evidence: synthetic fixture values, a discarded validation placeholder, a percent-encoded database name, and authorized or fixed filesystem paths; scanner settings remain unchanged.
